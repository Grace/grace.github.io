...
subtitle: "Amazon solved per-team attribution for callers that talk to Bedrock directly. Putting a gateway in front is how you give that up — and the gateway is the only thing that can give it back."
description: "Bedrock attributes cost to the IAM principal that made the call. Put a gateway in the path and every request bills to one identity — and the gateway is the only thing that can give the split back."
